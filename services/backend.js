...
(async () => {
  ...
  server.route({
    method: 'POST',
    path: '/quiz/newQuestion',
    handler: newQuestionHandler
  })

  server.route({
    method: 'GET',
    path: '/quiz/getQuestion',
...
function getQuestionState(){
  ...
  console.log('the current question state');
  console.log(JSON.stringify(response));
  return response;
}

function newQuestionHandler(req){
  console.log('received new question requset');
  const result = verifyAndDecode(req.headers.authorization);
  const payload = req.payload;
  if(payload){
    const {question: question, answers: answers} = payload; // this won't be from the paylaod. whtf si the paylaod. TODO
    setQuestionState(question, answers);
    return "success";
  }
  else {
    console.log("received request to newQuestion that did not contain a payload");
    return "fail"
  }
}

function getQuestionHandler(req,reply){
  const result = verifyAndDecode(req.headers.authorization);
